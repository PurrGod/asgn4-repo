...
def test_get_redirects_to_owning_shard(conductor: ClusterConductor, dir, log: Logger):
    with KVSTestFixture(conductor, dir, log, node_count=2) as fx:
        ...
        for i in range(40):
            k = f"probe-{i}"
            o, _ = _routed_put(fx, primaries, k, "v")
            assert o is not None, f"PUT '{k}' was not accepted by any shard"
        ...
        assert r.status_code == 307, (
            f"GET to wrong shard should 307, got {r.status_code}"
        )
        ...
        assert r.status_code == 200, (
            f"owning shard primary must serve directly, got {r.status_code} (no double redirect)"
        )
