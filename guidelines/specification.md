...
specified `key` across all servers. The response status code MUST be a 200 and
the service MUST reply within a timely manner (at most N seconds from receiving
the request) UNLESS replying that soon would violate strong consistency due to a
partition in the network or a crashed node. Once the response to a PUT request
has been sent it is considered acknowledged.
...
manner (at most N seconds from receiving the request) UNLESS replying would
